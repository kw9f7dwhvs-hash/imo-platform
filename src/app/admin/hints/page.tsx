...
export default function AdminHintsPage() {
  ...
  const fetchPool = () => {
    setLoading(true);
    fetch('/api/admin/hints-pool').then(r => r.json()).then(d => { setEntries(Array.isArray(d) ? d : []); setLoading(false); }).catch(() => setLoading(false));
  };
  useEffect(() => { fetchPool(); }, []);

  const handleAction = async (poolId: number, action: string) => {
    await fetch('/api/admin/hints-pool', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ poolId, action }) });
    fetchPool();
  };
  ...
  return (
    <AuthGuard requiredRole="admin">
      <div className="space-y-4">
        ...
        {loading ? <p className="text-gray-400">Loading...</p> : entries.length === 0 ? <p className="text-gray-500">No hints in pool.</p> : (
          <div className="space-y-3">
            {entries.map((e: any) => (
              <div key={e.id} className={'bg-white p-4 rounded-lg border ' + (e.status === 'standby' ? 'border-l-4 border-l-amber-400' : '')}>
                <div className="flex justify-between mb-2">
                  <div><p className="font-medium text-sm">{e.problem?.title || '#' + e.problemId}</p><p className="text-xs text-gray-500">By {e.submitter?.username}</p></div>
